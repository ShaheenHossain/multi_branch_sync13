...
class WarrantyDetail(models.Model):
    ...
    def check_invoice_paid(self):
        for rec in self:
            rec.is_invoice_paid = True if rec.invoice_id and rec.invoice_id.invoice_payment_state == 'paid' else False
    ...
    tag_ids = fields.Many2many('crm.lead.tag', string="Tags", readonly=True, states={'draft': [('readonly', False)]})
    product_id = fields.Many2one('product.product', string="Product", required=True, readonly=True, states={'draft': [('readonly', False)]})
    serial_id = fields.Many2one('stock.production.lot', string="Serial No.", readonly=True, states={'draft': [('readonly', False)]})
    ...
    def create(self, vals):
        ...
        return super(WarrantyDetail, self).create(vals)
    ...
    def onchange_sale_line(self):
        ...
        if self.sale_line_id:
            domain = [('id', '=', self.sale_line_id.product_id.id)]
    ...
    def onchange_template_id(self):
        if self.template_id:
            ...
            self.end_date = datetime.strptime(str(current_date), DEFAULT_SERVER_DATE_FORMAT).date() + relativedelta(months=months)
    ...
    def action_create_invoice(self):
        """
            Create invoice for thee warranty
        """
    ...
    def action_renew(self):
        current_date = fields.Date.today()
        end_date = datetime.strptime(str(self.end_date), DEFAULT_SERVER_DATE_FORMAT).date()
        warranty_start_date = current_date if self.state == 'expired' else end_date
        renew_id = self.copy(default={
                'start_date': warranty_start_date,
                'end_date': datetime.strptime(str(warranty_start_date), DEFAULT_SERVER_DATE_FORMAT).date() + relativedelta(months=self.template_id.warranty_renew_months),
                'parent_id': self.id,
    ...
    def scheduler_check_warranty(self):
        """
            Schedular for check warranty expiry
        """
        current_date = datetime.strptime(str(fields.Date.today()), DEFAULT_SERVER_DATE_FORMAT).date()
        for rec in self.search([('state', '=', 'running')]):
            if rec.end_date:
                end_date = datetime.strptime(str(rec.end_date), DEFAULT_SERVER_DATE_FORMAT).date()
                if current_date == end_date:
                    rec.action_expiry()
                before_expiry_date = end_date - timedelta(days=10)
